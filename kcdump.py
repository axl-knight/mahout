# The Python code in this cell is taken from https://github.com/axl-knight/mahout GitHub repository.
# The functions are to be used for calculating SSE and BSS from the cluster dump txt file.
#
# Ref: https://github.com/axl-knight/mahout/blob/main/kmeans-cluster-dump-analysis.py
#
...
distance_calculations = {'Cosine': cosine_distance, 'Squared_Euclidean': squared_euclidean_distance}
    ...
    with open(fpath, 'r') as f : # reading a txt file where the cluster dump is contained
        ...
        for text in f.readlines() :        
            ...
            if '[distance=' in text :
                ...
                sse = sse + float( text.split(': ')[1].replace('[distance=', '').replace(']', '') )**(2.0 if distance != 'SquaredEuclidean' else 1.0)
...
def mean_squared_between_distance(fpath, fpath_k1, distance) :
    ...
    for label in centroids.keys() :
        ...
        bss = bss + npoints * distance_calculations[distance](cm,centroid)**2.0
